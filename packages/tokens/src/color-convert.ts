...
import { oklch as oklchConverter } from 'culori'
...
export function hslToOklch(hsl: string): string {
  ...
  const hslColor = { mode: 'hsl' as const, h, s, l, ...(alpha ? { alpha: parseFloat(alpha) } : {}) }
  
  // Convert to OKLCH
  const oklch = oklchConverter(hslColor)
  if (!oklch) throw new Error(`Failed to convert HSL to OKLCH: ${hsl}`)
 ...
 */
export function convertColorsToOklch<T extends Record<string, any>>(colorObj: T): T {
