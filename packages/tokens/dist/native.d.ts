export declare const nativeTokens: {
    readonly colors: {
        readonly darkNavy: string;
        readonly darkBackground: string;
        readonly midNavy: string;
        readonly deepBlue: string;
        readonly lightBlue: string;
        readonly white: string;
        readonly success: string;
        readonly warning: string;
        readonly error: string;
        readonly info: string;
        readonly purple: string;
        readonly glassWhite: string;
        readonly overlayDark: string;
        readonly subtleOverlay: string;
        readonly borderLight: string;
        readonly borderBlue: string;
        readonly darkGlass: string;
        readonly gradientListItem: string;
        readonly primary: string;
        readonly secondary: string;
        readonly background: string;
        readonly surface: string;
        readonly border: string;
        readonly whiteText: {
            readonly primary: string;
            readonly secondary: string;
            readonly tertiary: string;
            readonly disabled: string;
        };
    };
    readonly gradients: {
        readonly cyanPurple: {
            readonly colors: readonly ["#06B6D4", "#3B82F6", "#8B5CF6"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly purpleBlue: {
            readonly colors: readonly ["#9333ea", "#3b82f6", "#8b5cf6"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly pinkPurple: {
            readonly colors: readonly ["#ec4899", "#8b5cf6", "#6366f1"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly cyanVibrant: {
            readonly colors: readonly ["#00c4ff", "#00ffff", "#00aaff", "#00e0ff", "#00c4ff"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly loginCyan: {
            readonly colors: readonly ["#00c4ff", "#00ffff", "#00aaff", "#00e0ff", "#00c4ff"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly cosmic: {
            readonly colors: readonly ["#1a1a2e", "#16213e", "#0f3460"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
        readonly cosmicHover: {
            readonly colors: readonly ["#1f2233", "#1a2a47", "#0f3c6e"];
            readonly start: {
                readonly x: 0;
                readonly y: 0;
            };
            readonly end: {
                readonly x: 1;
                readonly y: 1;
            };
        };
    };
    readonly spacing: {
        readonly xs: 4;
        readonly sm: 8;
        readonly md: 16;
        readonly lg: 24;
        readonly xl: 32;
        readonly xxl: 48;
        readonly xxxl: 64;
        readonly xxxxl: 96;
    };
    readonly typography: {
        readonly hero: {
            readonly fontSize: 56;
            readonly lineHeight: 56;
            readonly fontWeight: "700";
        };
        readonly h1: {
            readonly fontSize: 48;
            readonly lineHeight: 52.8;
            readonly fontWeight: "700";
        };
        readonly h2: {
            readonly fontSize: 36;
            readonly lineHeight: 43.2;
            readonly fontWeight: "600";
        };
        readonly h3: {
            readonly fontSize: 24;
            readonly lineHeight: 31.2;
            readonly fontWeight: "600";
        };
        readonly bodyLarge: {
            readonly fontSize: 18;
            readonly lineHeight: 28.8;
            readonly fontWeight: "400";
        };
        readonly body: {
            readonly fontSize: 16;
            readonly lineHeight: 24;
            readonly fontWeight: "400";
        };
        readonly bodySmall: {
            readonly fontSize: 14;
            readonly lineHeight: 21;
            readonly fontWeight: "400";
        };
        readonly caption: {
            readonly fontSize: 12;
            readonly lineHeight: 16.8;
            readonly fontWeight: "400";
        };
        readonly button: {
            readonly fontSize: 16;
            readonly lineHeight: 16;
            readonly fontWeight: "600";
        };
    };
    readonly shadows: {
        readonly subtle: {
            readonly shadowColor: "#000";
            readonly shadowOffset: {
                readonly width: 0;
                readonly height: 2;
            };
            readonly shadowOpacity: 0.1;
            readonly shadowRadius: 4;
            readonly elevation: 2;
        };
        readonly default: {
            readonly shadowColor: "#000";
            readonly shadowOffset: {
                readonly width: 0;
                readonly height: 4;
            };
            readonly shadowOpacity: 0.15;
            readonly shadowRadius: 12;
            readonly elevation: 4;
        };
        readonly elevated: {
            readonly shadowColor: "#0f3460";
            readonly shadowOffset: {
                readonly width: 0;
                readonly height: 8;
            };
            readonly shadowOpacity: 0.3;
            readonly shadowRadius: 25;
            readonly elevation: 8;
        };
        readonly deep: {
            readonly shadowColor: "#000";
            readonly shadowOffset: {
                readonly width: 0;
                readonly height: 20;
            };
            readonly shadowOpacity: 0.4;
            readonly shadowRadius: 60;
            readonly elevation: 16;
        };
    };
    readonly radii: {
        readonly xs: 4;
        readonly sm: 6;
        readonly md: 8;
        readonly default: 12;
        readonly lg: 12;
        readonly xl: 16;
        readonly xxl: 24;
        readonly full: 9999;
    };
};
